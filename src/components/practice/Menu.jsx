...
function Menu({ img, name, price, onClick }) {
  return (
    <div
      ...
      onClick={() => onClick({ img, name, price })}
    >
      <img className='w-32 h-32' src={img} />
      <span className='text-[1.625rem] font-medium w-45 text-center'>{name}</span>
      <span className='text-[1.625rem] font-medium text-[#B01414]'>{price}원</span>
    </div>
  )
}
